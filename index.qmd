...
joined_data |>
  ...
  labs(
    title = "Total Arrests vs. ICE Budget",
    x = "ICE Budget (in millions)",
    y = "Arrests"
  ) +
  scale_y_continuous(labels = comma) +
...
BookIns |>
  ...
  geom_point(size = 3, color = "blue4") +
  geom_line(color = "blue4") +
  geom_text(aes(label = Year), angle = 0, hjust = 1.5, vjust = 1.4, size = 2) +
  labs(title = "Total Arrests vs. ICE Budget",
       x = "ICE Budget (in millions)",
       y = "Arrests", 
       caption = "Source: ICE Detention Statics from ice.gov") +
...
Facilities <- Facilities %>%
  mutate(type_detailed = case_when(
    ...
    TRUE ~ "Other"  # Everything else becomes 'Other'
  ))

# View the changes
head(Facilities$type_detailed)

# Optionally, check how the categories are now distributed
...
```{r}
pal <- colorFactor(palette = "Set2", domain = Facilities$type_detailed)
